...
datadir = r"C:\Users\juanxin\PycharmProjects\SelfTask-GNN-master\data"

def parse_index_file(filename):
    """Parse index file."""
    index = []
    for line in open(filename):
        index.append(int(line.strip()))  # Python strip() 方法用于移除字符串头尾指定的字符（默认为空格或换行符）或字符序列
    return index
...
def load_citation(dataset_str="cora", normalization="AugNormAdj", porting_to_torch=True,data_path=datadir, task_type="full"):
    ...
    objects = []
    for i in range(len(names)):
        print("{}.{}".format(dataset_str.lower(), names[i]))
        data_path = r"C:\Users\隽歆\PycharmProjects\SelfTask-GNN-master\data"
...
def split_by_degree(adj, thresholds=[1, 10]):
    ...
    k = len(high)
    high_ = np.random.permutation(high)[:k]    # 有问题 ################################################3
